...
export default function HorizontalNavBar({ isMenuOpen, toggleSideNav }) {
  return (
    <nav style={{ backgroundImage: `url('assets/istockphoto-1020105066-612x612.jpg')` }}
      className='flex bg-cover backdrop-blur-sm min-w-screen w-screen fixed top-0 justify-around items-center bg-green-100 h-20 z-10' >
      <div className='w-1/5 xs:w-2/3 app-logo  flex justify-between items-center '>
        <span className='pr-4 block md:hidden lg:hidden xs:pl-2 w-1/5 text-white text-3xl' onClick={toggleSideNav}>{!isMenuOpen ? <RiMenuUnfoldLine /> : <RiMenuUnfold2Line />}</span>
      ...
      <div className='w-1/5 flex justify-between xs:justify-center items-center px-3 login-info'>
